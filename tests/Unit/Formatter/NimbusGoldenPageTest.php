...
/**
 * Byte equality against a committed page, for the nimbus dialect.
 *
 * The same five declarations as GoldenPageTest, rendered as MDX. Whitespace
 * is load-bearing here for a second reason: MDX ends a component at a blank
 * line, so a template that gains or loses one does not merely move the
 * output - it stops the page from compiling.
 *
 * The expectation also pins the anchors. Every heading id in this file must
 * match the one in `phalcon_sample.md`, because the published pages carry
 * them and the prose pages link to them.
 *
 * Regenerate deliberately, never to make a red test green:
 *
 *     docker exec -e UPDATE_GOLDEN=1 quill-8.1 \
 *         vendor/bin/phpunit -c resources/phpunit.xml.dist --filter NimbusGoldenPageTest
 *
 * Then read the diff. An intentional markup change shows up there; an
 * accidental one shows up there too, which is the point.
 */
final class NimbusGoldenPageTest extends TestCase
{
    public function testThePageMatchesTheCommittedExpectation(): void
    {
        ...
        $path = dirname(__DIR__, 2) . '/Fixtures/golden/phalcon_sample.mdx';
